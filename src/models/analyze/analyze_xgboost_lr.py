# Analyze learning_rate in XGBoost
import os
...
def analyze_xgboost_lr(X, y, learning_rates=None):
    ...
    for lr in learning_rates:
        clf = XGBClassifier(learning_rate=lr, eval_metric='logloss', random_state=42)
    ...
    plt.savefig("./results/xgboost_lr.png", dpi=150, bbox_inches='tight')
    plt.show()


def main():
    # RESULT:
    # Best Train Accuracy: 0.7773 at learning_rate 0.3
    # Best Test Accuracy: 0.7393 at learning_rate 0.1
    df = pd.read_csv("../../../data/processed/cardio_train_cleaned.csv", sep=",")
    y = df['cardio']
    X = df.drop(columns=['cardio'])
    analyze_xgboost_lr(X, y)
