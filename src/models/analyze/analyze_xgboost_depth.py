...
def analyze_xgboost_depth(X, y, max_depth_range=range(1, 11)):
    ...
    for depth in max_depth_range:
        clf = XGBClassifier(max_depth=depth, eval_metric='logloss', random_state=42)
    ...
    plt.savefig("./results/xgboost_depth.png", dpi=150, bbox_inches='tight')
    plt.show()


def main():
    # RESULT:
    # Best Train Accuracy: 0.8945 at depth 10
    # Best Test Accuracy: 0.7382 at depth 3
    df = pd.read_csv("../../../data/processed/cardio_train_cleaned.csv", sep=",")
    y = df['cardio']
    X = df.drop(columns=['cardio'])
    analyze_xgboost_depth(X, y)
